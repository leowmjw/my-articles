...
- Walk through the assembly of a "Joined Up" Data Application using a real-life scenario, concrete examples and clear explanation of the details.
...
In light of this; how can we make it accessible to the general public; and empower proper analysis of the budget and the accountability of its execution?

Every October, the Malaysian Government presents its Budget for the upcoming year.  The data is published as non-machine readable PDF separated by specific Ministries.  Detailed information about Ministries and their associated Ministers are scattered around various Government sites.

The goal is to come up with a simple Proof-of-Concept (PoC) "Joined Up" Data Application (using the individual silo-ed data below) that allows the user to choose the Ministry to be scrutinized against its published Budget in an easy, interactive and user-friendly manner.
...
The final application with all the components assembled as per above looks as per below:
   ...
   ![VueJS Final Demo](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/VueJS-Final-Demo.png)

### Detailed Analysis of Components

Let's have a look at the individual Components; briefly described by the High Level Architecture Sketch above; in more details.  
...
However, only the first component is actually rendered until the user takes further action to select a Ministry. 
   ...
   ![VueJS Final Demo Code](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/VueJS-Final-Demo-Code.png)

#### Ministry Selection and Published Budgets Component

The first component which allows the user to select the Ministry to be analyzed is made out of two section: Ministry Selection (marked as red) and Available Budgets (marked as orange); that will render the full list of available Published Budgets that the user can select for the next step in visualization.    

   ![Demo Show Ministry Available Budgets](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Show-Ministry-Available-Budgets.png)
...
Once a Ministry has been selected by the user, the Components to show the Ministry Details along with the Budget Details now becomes activated:
   ...
   ![Code - Demo Show Ministry Available Budgets](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Show-Selected-Ministers-Available-Budgets-Code.png)


#### Minister Details Component

The Minister Details Component will pull out the detailed data from the PopIt API of the chosen Ministry.  The data returned will be filtered to only members of the "Fifth Cabinet of Najib Razak" to match up with the time period of the budget.

In the codebase seen earlier, this Component is referred to with the element named **<related_ministries_info>**: 
     
   ![abc](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Minister-Chosen-Ministry.png)
    
The Component code layout is a fairly standard view of the Ministry details along with the high quality headshot image of the Ministry and any Deputies.
        
   ![abc](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Minister-Chosen-Ministry-Code.png)
...
Users that need more details can click on the link to **"OSNext"** where the full capabilities of the OpenSpending portal is available: such as more Visualization options (e.g Pivot Table, San Key) and more powerful drill-down (different measures, different filters).
   ...
   ![BabbagePackage](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-BabbagePackage-HighLevel-Code.png)
...
The element **<babbage_package>** itself is made out of the 3 Sub-Components for visualizing the data in 3 forms: TreeMap, BubbleTree and Pie.
   ...
   ![TreeMap](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-BabbagePackage-LowLevel-Code.png)
...
The TreeMap Sub-Component has the visual area size to represent the ratio of budgeted amount vis-a-vis the other line items in the same level of hierarchy.  This is expressed further down in clear TreeMap List which can be sorted by the raw amount as well as by its relative ratio:

   ![TreeMap](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Components-TreeMap.png)
...
The BubbleTree Sub-Component also uses the relative size of the bubbles to indicate the ratio of values for any particular hierarchy level of the Measure:
   ...
   ![BubbleTree](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Components-BubbleTree.png)

The Pie Sub-Component is the most mundane of the three, a standard Pie Chart:
    
   ![Pie](https://raw.githubusercontent.com/leowmjw/my-articles/master/SINARPROJECT/OPENSPENDING/JOINED/IMAGES/Demo-Components-Pie.png)


### Further Ideas

We can also use Components to wrap around other data sources like those from our partners MySociety [FixMyStreet](http://www.fixmystreet.org/) (FMS).  

In the context of Malaysia, we can provide a Component that allows the user to relate issues reported by our locally-branded FMS - [Aduanku](https://www.aduanku.my/) against the accountable parties at all levels of government; whether it is the Member of Parliament, Member of State Assembly or the Local Councillor.  

Again, the "Joined Up" Data will involve composing against data extracted from the PopIt API. 

Finally; since Components are pretty much standalone, especially when implemented via VueJS; it makes it much easier to be incorporated into a normal static content site.  For example: A "Public Hospital Budget and Expenditure" Component used to show the effect of the Ministry of Health budget cuts in an online newspaper article.  
...
The message is clear: Do not try to solve any problem from scratch; reuse the available Components and share any new Components created.
...
- Source code for the sample application: <https://github.com/leowmjw/vue-babbageui-demo>
- Source code for the Base Component: <https://github.com/openspending/babbage.ui>
